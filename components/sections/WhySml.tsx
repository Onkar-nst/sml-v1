  ...
  return (
    <section className="relative z-10 pt-12 md:pt-[6vw] lg:pt-[4.5rem] pb-18 md:pb-[9vw] lg:pb-28" id="why">
      ...
      {/* below lg the row scrolls inside the container, so cut-off cards clip
          ...
          The row is horizontal-only on touch: touch-pan-x locks a swipe that
          starts on the cards to the horizontal axis, and overflow-y-hidden
          stops the box itself from picking up a vertical scroll of its own —
          declaring overflow on one axis makes the browser treat the other as
          `auto`, which is enough for a tall card to drift under the finger.
          Mandatory snapping lands each swipe on a card, and overscroll-x-contain
          keeps a fling at either end from chaining out to the browser's back
          gesture. All of it is dropped at lg, where the row stops scrolling and
          becomes a plain five-up grid. */}
      <div className="wrap">
        <div
          className="flex gap-4 md:gap-[1.7vw] overflow-x-auto overflow-y-hidden py-2 pb-5 pr-4 md:pr-[1.7vw] snap-x snap-mandatory touch-pan-x overscroll-x-contain scrollbar-none lg:grid lg:grid-cols-5 lg:gap-4 xl:gap-5 lg:overflow-visible lg:overflow-y-visible lg:pr-0 lg:touch-auto lg:snap-none"
          aria-label="What SML offers"
        >
          {WHY_CARDS.map((item, i) => {
            const Icon = ICONS[item.icon]
            return (
              /* One and three-quarter cards to a screen on phones: the row
                 shows a card, a gap, and three quarters of the next. Because
                 the trailing pr-4 matches the gap, 100% / 1.75 of the padded
                 box lands the cut on the gutter at every phone width. */
              /* the whole card is the link into Our Solutions — it opens that
                 range's chip, which is what the card is a shorthand for */
              <a
                key={item.title}
                href={`#${item.family}`}
                aria-label={`${item.title} — see the range in Our Solutions`}
                className="relative overflow-hidden flex-none w-[calc(100%/1.75)] sm:w-[clamp(240px,calc((100%-3*clamp(1rem,1.7vw,1.5rem))/4.4),330px)] lg:w-auto snap-start px-6 pt-[1.75rem] pb-[3.25rem] lg:px-5 lg:pt-6 lg:pb-9 xl:px-6 border border-[#193174]/9 rounded-2xl bg-white/92 backdrop-blur-md shadow-sm transition-all duration-350 ease-out hover:-translate-y-1.5 hover:shadow-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b5cf9a] focus-visible:ring-offset-2 focus-visible:ring-offset-transparent group"
                data-reveal
              ...
              >
                ...
                <img
                  ...
                  loading="lazy"
                  draggable={false}
                  className="pointer-events-none select-none absolute inset-0 w-full h-full object-cover opacity-50 transition-all duration-500 ease-out group-hover:opacity-70 group-hover:scale-106"
                />
                ...
                <div className="relative">
                  ...
                  <p className="m-0 text-[0.93rem] lg:text-[0.86rem] xl:text-[0.9rem] leading-[1.6] text-[#193174]/75">{item.body}</p>
                </div>
              </a>
            )
          })}
